...
//		System.out.println(this.venId);
	...
	public void menuItemChanged(ValueChangeEvent e){
		//assign new value to localeCode
		this.menuItem = e.getNewValue().toString();
		Map<String, Object> sessionMap =
				FacesContext.getCurrentInstance().getExternalContext().getSessionMap();
	}
	...
	public Customer searchCustomerById(int cusid) {
		...
		session = sf.openSession();
		Criteria criteria = session.createCriteria(Customer.class);
		criteria.add(Restrictions.eq("cusid", cusid));
	...
	public String customerLoginDao(Customer customer) {
		...
		if(count==1) {
			...
			return "CustomerDashboard.jsp?faces-redirect=true";
		}else {
